...
面向本地 Linux 环境的 SAR 元数据归纳、PostGIS 入库和 Cesium 查询接口。当前支持 GF3、StriX、Umbra、Capella、ICEYE、WANG，以及普通目录、ZIP、TAR、TAR.GZ、TGZ 数据容器。
...
忽略 4 个 ICEYE 辅助 JSON。GF3 和 WANG 样例缺少部分影像资产，因此是 `PARTIAL`，
其余 4 个主产品是 `READY`。
